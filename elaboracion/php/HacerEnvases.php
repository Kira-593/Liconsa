...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'undo_signature') {
    ...
    $update_query = "UPDATE e_envases_rotos SET firma_usuario = '', fecha_firma = NULL = '' WHERE id = $ID";
    
    if (mysqli_query($link, $update_query)) {
        echo "<script>alert('Firma deshacha correctamente. El formulario está listo para editar.'); window.location.href = 'actualizarEnvases.php?id=$ID';</script>";
    } else {
...
if (empty($ID)) {
    echo "<script>
        alert('Error: ID del registro no recibido.');
        window.history.back();
    </script>";
    include "Cerrar.php";
    exit;
        ...
        <?php
            ...
            if ($ejecucion_exitosa) {
                if ($filas_afectadas > 0) {
                    $mensaje = "Actualización de Formulario correcta";
                    if ($firma_realizada) {
